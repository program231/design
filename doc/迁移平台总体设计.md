...
本文档在分析 DataX、Apache Flink 与 OceanBase OMS 三套系统的设计基础上，提出一套**自研异构数据迁移平台**的总体架构设计。

**文档定位**：本平台为一体化自研系统设计，**借鉴**三套参考系统的架构模式与模块划分（如 OMS 的产品化迁移流程、DataX 的 Reader-Channel-Writer 管道、Flink 的 Checkpoint 容错机制），在平台内重新实现，**不将 DataX、Flink 作为外部引擎调用或嵌入**。
