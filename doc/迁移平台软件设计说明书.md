...
| **编制日期** | 2026-07-09 |
| **文档状态** | 评审稿 |
| **适用范围** | 自研异构数据迁移平台（非 DataX/Flink 集成方案） |
...
| 版本 | 日期 | 修订人 | 修订说明 | 状态 |
|------|------|--------|----------|------|
| v1.0 | 2026-07-09 | — | 初稿：基于总体设计 v1.1 编制 SDD，完成架构、18 模块、流程与非功能设计 | 已归档 |
| v2.0 | 2026-07-09 | — | **重大修订**：明确为自研迁移平台设计，借鉴 DataX/Flink/OMS 架构思想，非外部引擎集成方案 | 评审稿 |
...
本文档为**自研异构数据迁移平台**的软件设计说明书（SDD）。

设计过程中，团队深入研读了 [DataX 设计框架与模块分析](DataX设计框架与模块分析.docx)、[Flink 设计框架与模块总结](Flink设计框架与模块总结.docx) 及 [OceanBase OMS](OMS架构参考.md) 官方架构文档，**提炼其架构模式、模块划分与运行机制**，用于指导本平台的设计。**本平台为一体化自研系统，不将 DataX、Flink 作为外部引擎调用或嵌入。**
...
| FullImport | 平台自研全量数据导入组件（借鉴 DataX Reader-Channel-Writer 管道） |
| Store | 源端增量日志拉取、解析与持久化组件 |
| IncrSync | 增量 Record 应用至目标端组件（借鉴 Flink 流式运行时与 Checkpoint 机制） |
| FullVerification | 源/目标全字段对比校验组件 |
| Supervisor | 组件健康监控与 HA 触发代理 |
| Record | 统一数据抽象：全量行为 Column 行模型，增量为 DML/DDL/HB |
| Checkpoint | 平台增量运行时的分布式一致性快照，用于故障恢复 |
...
| 3 | DataX 设计框架与模块分析 | **设计借鉴**：Framework+Plugin、Reader-Channel-Writer、Task 调度 |
| 4 | Flink 设计框架与模块总结 | **设计借鉴**：分布式运行时、Checkpoint、Connector SPI |
| 5 | [OMS架构参考](OMS架构参考.md) | **设计借鉴**：产品化迁移流程、组件划分、HA 模型 |
...
企业在数据库升级、云迁移、架构转型（集中式 → 分布式）过程中，普遍面临异构数据源、有限停机窗口、全增量衔接一致性及多阶段运维复杂度等挑战。

本平台的建设思路是：**设计一套完整的自研迁移平台**，而非在 DataX、Flink 等开源组件之上做二次封装。在方案设计阶段，对以下三套成熟系统进行了架构级分析：

| 参考系统 | 借鉴内容 | 本平台对应 |
|----------|----------|------------|
| **OceanBase OMS** | 产品化迁移流程、DBCat/Store/IncrSync 组件划分、HA 模型 | 服务接入层 + 流程编排层 + 执行组件 |
| **Alibaba DataX** | Framework+Plugin、Reader-Channel-Writer、Task/TaskGroup 调度、脏数据与流控 | FullImport + SyncWorker + Reader/Writer 插件 |
| **Apache Flink** | JobManager/TaskManager、Checkpoint、Connector SPI、Mailbox 单线程模型 | StreamRuntime + IncrSync + LogAdapter 插件 |

三套系统各有侧重，本平台取其精华、补其短板，形成**结构 → 全量 → 增量 → 校验 → 切换**的端到端自研能力。
...
1. **自研一体化**：全量引擎、增量引擎、编排控制台均为平台自有模块，不依赖外部 DataX/Flink 进程。
2. **借鉴不调用**：吸收 OMS 产品骨架、DataX 全量管道、Flink 流式容错等**设计模式**，在平台内重新实现。
3. **分层解耦**：服务接入层、流程编排层、执行引擎层、基础设施层职责清晰，各层可独立演进。
4. **插件即插即用**：Reader/Writer/LogAdapter/TypeMapper 均为平台自有 SPI，按 plugin.json 注册加载。
5. **流程驱动**：以状态机编排迁移阶段（借鉴 OMS），子任务可并行、可重试、可回滚。
6. **统一数据模型**：增量 Record 抽象为 DML/DDL/HB（借鉴 OMS），全量行数据采用统一 Column 模型（借鉴 DataX）。

### 2.4 设计借鉴说明

本节说明三套参考系统与本平台的**设计层关系**（非集成关系）：

```text
┌─────────────────────────────────────────────────────────────┐
│                    自研迁移平台（本文档）                      │
├──────────────┬────────────────────┬───────────────────────────┤
│ 借鉴 OMS     │ 借鉴 DataX         │ 借鉴 Flink                │
│ · 三层产品架构│ · Framework+Plugin│ · 分布式运行时             │
│ · 迁移阶段流程│ · Reader-Channel- │ · Checkpoint 容错         │
│ · DBCat/Store│   Writer 管道     │ · Connector/LogAdapter SPI│
│ · IncrSync   │ · Task/TaskGroup  │ · 控制面/数据面分离        │
│ · HA/Supervisor│ · 脏数据/流控   │ · Slot 资源模型           │
└──────────────┴────────────────────┴───────────────────────────┘
```

| 借鉴来源 | 借鉴的设计要素 | 本平台实现模块 |
|----------|----------------|----------------|
| OMS | 端到端迁移产品流程、组件命名与职责、DML/DDL/HB Record、Supervisor+HA | 控制台、工作流、DBCat、Store、IncrSync、FullVerification |
| DataX | 星型链路、插件 ClassLoader 隔离、1:1 Reader-Writer Task、Channel 背压流控、Communication 统计 | FullImport、SyncWorker、Reader/Writer 插件、Channel 传输层 |
| Flink | 作业提交与调度分离、Checkpoint 分布式快照、Source/Sink 连接器框架、算子链减少开销 | StreamRuntime、IncrSync、LogAdapter、Checkpoint 协调器 |

### 2.5 能力定位

| 能力维度 | 本平台（自研） |
|----------|----------------|
| 结构迁移 | DBCat（Schema 转换引擎） |
| 全量迁移 | FullImport（自研，借鉴 DataX 管道模型） |
| 增量同步 | Store + IncrSync（自研，借鉴 Flink 流式运行时） |
| 数据校验 | FullVerification |
| 可视化运维 | 统一控制台 + OpenAPI |
| 目标端 | 开放多种数据库，不绑定单一厂商 |

与 OMS、DataX、Flink 的详细对照见 [9.1 设计借鉴对照表](#91-设计借鉴对照表)。
...
模块按职责划分为五组：**管理与接入**、**核心服务**、**执行组件**、**插件层**、**平台运行时**。控制台经 API 网关访问任务编排服务；编排服务驱动工作流，工作流调度各执行组件；执行组件通过平台自有插件与 SyncWorker/StreamRuntime 完成数据搬运。
...
两者通过调度器解耦：控制平面下发作业规格与调度指令，数据平面在平台自有 Worker 节点池上执行，便于独立扩缩容与故障隔离。
...
All-in-One 节点集成控制台、API、编排服务、SyncWorker、StreamRuntime 与元数据库，适用于开发调试与 PoC 验证。
...
| 全量执行 | 自研 SyncWorker + Reader/Writer 插件 | 借鉴 DataX 管道模型，平台内实现 |
| 增量执行 | 自研 StreamRuntime + LogAdapter 插件 | 借鉴 Flink 运行时与 Checkpoint，平台内实现 |
...
将迁移流程建模为有向阶段图 + 子任务状态机，对标 OMS 流程编排层设计思想。每个 SubTask 映射到具体执行组件（DBCat、FullImport 等），由工作流负责就绪判定、并行拆分与失败重试策略。
...
| 位点管理 | 全量 `startPosition`、Store 消费位点、StreamRuntime Checkpoint 引用 |
| 映射元数据 | 表/列名映射、类型映射结果 |
...
全量数据迁移执行组件，平台**自研**实现，负责将源端存量数据高吞吐迁移至目标端。

**设计思路**

借鉴 DataX 的 Framework + Plugin 架构与 Reader-Channel-Writer 管道模型，在平台内实现全量同步引擎，**不调用外部 DataX 进程**。核心设计要点来自 DataX 分析文档：

- **星型链路**：异构 N×N 同步简化为「各源 Reader → 平台 Channel → 各目标 Writer」
- **严格 1:1 Task 配对**：每个全量 Task 固定 1 Reader : 1 Channel : 1 Writer
- **Writer 先启动**：防止 Reader 提前结束导致 Channel 数据未写完
- **Task/TaskGroup 调度**：Job 切分为 N 个 Task，按 Channel 数分组并发执行

同时借鉴 OMS Full-Import 的「切片 → 读取 → 加工 → 应用」四阶段与切片位点 ACK 机制，保障重启后数据完整。

**功能职责**

| 功能 | 说明 |
|------|------|
| 作业编排 | 根据映射元数据生成全量作业配置 |
| 表级切片 | 按主键/分区切分 Task |
| 管道传输 | Reader → Channel → Writer 流水线 |
...
- 上游：工作流服务、调度器
- 下游：SyncWorker、Reader/Writer 插件
- 依赖：元数据服务、数据源注册中心

**关键机制**

- Channel 采用内存队列 + 字节容量背压（借鉴 DataX MemoryChannel）
- 插件 ClassLoader 隔离加载（借鉴 DataX JarLoader）
- 全量开始时写入 `startPosition` 供 Store 衔接

**异常与容错**

- Task 级 Failover 自动重试（可配置 maxRetryTimes）
- 脏数据超限按 errorLimit 中止并告警
- 不支持 HA：失败后由工作流触发人工重试
...
- 依赖：元数据服务、数据源注册中心、LogAdapter 插件
- 协作：Supervisor、HA 组件
...
增量同步执行组件，将 Store 产生的 Record 应用至目标端，由平台自研 **StreamRuntime** 调度运行。

**设计思路**

借鉴 OMS Incr-Sync 的 Record 应用模型与 Flink 的分布式流式运行时设计，在平台内实现增量引擎，**不依赖外部 Flink 集群**：

- **借鉴 OMS**：DML/DDL/HB Record 抽象；有主键表幂等写入；无主键表事务表机制（V2）
- **借鉴 Flink**：StreamCoordinator（类 JobManager）+ StreamWorker（类 TaskManager）进程模型；Checkpoint 分布式快照保障断点续传；LogAdapter SPI（类 Connector）对接各源端日志
...
- 上游：Store
- 下游：StreamRuntime、目标数据库
- 依赖：元数据服务、LogAdapter 插件
- 协作：Supervisor、HA 组件

**关键机制**

- 统一 Record：DML / DDL / HB
- Checkpoint 协调器周期性注入 Barrier 并持久化状态（借鉴 Flink Chandy-Lamport 快照）
- DDL 与 DML 分流处理，防止结构不一致

**异常与容错**

- **支持 HA**：进程异常重启（10 分钟冷却）；节点宕机跨机重建
- 宕机恢复后可能存在短暂双写，依赖目标端幂等
- 无主键表：预检查提示一致性风险
...
接收编排/工作流下发的作业调度请求，结合资源池状态将全量、增量等作业分配到合适 Worker 节点。

**设计思路**

对标 OMS 调度服务（8088）与 Flink ResourceManager 的调度思想，调度器是控制平面与数据平面的**衔接枢纽**。支持优先级、公平、亲和与弹性策略。
...
- 上游：工作流服务、任务编排服务
- 下游：资源池、SyncWorker、StreamRuntime
- 依赖：元数据服务（作业状态）
...
将 Worker 节点池抽象为统一资源视图，供调度器消费。参考 OMS 资源池管理与 Flink Slot 资源模型，支持按租户/项目限制 CPU、内存与并发作业数。
...
- 上游：调度器
- 下游：SyncWorker 节点池、StreamRuntime Worker
- 依赖：元数据服务、监控告警
...
| 资源类型 | 分配单位 |
|----------|----------|
| SyncWorker | 进程/容器，每全量 Job 一个 Worker |
| StreamWorker Slot | Slot，每增量作业若干 Slot |
| CPU/内存 | 租户配额 |
...
融合平台 Communication 统计、StreamRuntime Metrics 与 Supervisor 心跳，写入时序库（Prometheus/InfluxDB），经 Grafana 大盘展示。AlertManager 对接邮件、钉钉、Webhook。
...
平台自研的数据读取与写入插件体系，借鉴 DataX SPI 设计，为 FullImport 提供异构数据源接入能力。

**设计思路**

参考 DataX 插件分析文档，定义平台自有 Reader/Writer SPI：

- `Reader.Job`：`init()` → `split(adviceNumber)` → `post()` → `destroy()`
- `Reader.Task`：`init()` → `startRead(RecordSender)` → `post()` → `destroy()`
- `Writer` 对称定义

通过 `plugin.json` 注册，JarLoader 实现 ClassLoader 隔离（借鉴 DataX）。**插件为平台自有实现，非 DataX 官方插件直接复用。**
...
- 被依赖：FullImport、SyncWorker
- 依赖：数据源注册中心、插件注册中心

**关键机制**

- `plugin.json` 元信息与作业模板
- 严格 1:1 Reader-Writer Task 配对
- 支持 MySQL、Oracle、PostgreSQL、OceanBase 等（详见 6.2 节）
...
平台自研的源端变更日志适配器，借鉴 Flink Connector SPI（FLIP-27）设计，为 Store 提供 binlog/redo 等日志拉取能力。

**设计思路**

参考 Flink `Source/Sink + SplitEnumerator` 接口思想，定义平台 LogAdapter SPI：

- `LogReader`：订阅源端变更日志，输出统一 Record
- `SplitEnumerator`：管理日志分片（如按库表/分区）
- `OffsetCommitter`：位点提交与 Checkpoint 对齐

**功能职责**

| 功能 | 说明 |
|------|------|
| 变更捕获 | MySQL Binlog、Oracle Redo、OB Liboblog 等 |
| Split 管理 | 分片枚举与分配 |
| 位点提交 | 与 StreamRuntime Checkpoint 对齐 |
...
- SPI 注册：`META-INF/services/...LogAdapterFactory`
- 与统一 DML/DDL/HB 模型衔接
- 事务边界解析保证 Record 顺序语义

**异常与容错**

- 位点不可用：回退至最近有效 Checkpoint
- Schema 不兼容：暂停作业并告警
- 适配器版本不匹配：启动前兼容性校验

---

#### 4.5.3 SyncWorker（全量执行运行时）

**模块概述**

平台自研全量迁移 Worker 运行时，借鉴 DataX Engine + JobContainer + TaskGroupContainer 设计，在 Worker 进程内执行 FullImport 作业。

**设计思路**

参考 DataX 核心模块分析，在平台内实现等价运行时：

```text
SyncEngine (入口)
  └── JobContainer (作业级管理)
        └── Scheduler (调度)
              └── TaskGroupContainer (Channel 池)
                    └── TaskExecutor (1 Reader : 1 Channel : 1 Writer)
```

平台屏蔽底层调度细节，以「全量作业」抽象对外。继承 Channel 背压、脏数据与 Communication 统计机制。

**功能职责**

| 功能 | 说明 |
|------|------|
| 作业解析 | 加载作业配置，初始化 JobContainer |
...
- 编排层自动生成作业配置，用户无需编写底层配置

**异常与容错**

- Task Failover：可配置 maxRetryTimes
- OOM：限制 Channel 缓冲与并发度
- 进程崩溃：调度器在其他 Worker 重新提交

---

#### 4.5.4 StreamRuntime（增量流式运行时）

**模块概述**

平台自研增量流式运行时，借鉴 Flink JobManager/TaskManager 分布式进程模型，调度 IncrSync 作业。

**设计思路**

参考 Flink 运行时架构，在平台内实现：

| Flink 概念 | 本平台实现 |
|------------|------------|
| JobManager | StreamCoordinator（作业协调、Checkpoint 触发） |
| TaskManager | StreamWorker（执行 IncrSync Task） |
| Slot | WorkerSlot（资源隔离单元） |
| Checkpoint | 平台 Checkpoint 协调器 + 状态后端 |
| Savepoint | 手动一致性快照，用于升级与恢复 |

**功能职责**

| 功能 | 说明 |
|------|------|
| 作业提交 | 接收 IncrSync 生成的执行图 |
| 资源管理 | StreamCoordinator/StreamWorker、Slot 分配 |
| Checkpoint | 周期性分布式快照 |
| Savepoint | 版本升级与手动恢复 |
| 监控集成 | 指标上报至平台控制台 |
...
- 上游：调度器、IncrSync
- 下游：LogAdapter 插件、目标数据库
- 依赖：资源池、元数据服务（位点引用）

**关键机制**

- Checkpoint 持久化至分布式存储
- 并行度与 Slot 数匹配
- Mailbox 单线程模型处理事件（借鉴 Flink，避免锁竞争）

**异常与容错**

- StreamWorker 失败：自动重启 Task
- StreamCoordinator HA：ZK/etcd 选主
...
4. StreamRuntime Checkpoint 持久化 IncrSync 消费位点，与 Store 协同
...
| 全量迁移 | **表级并行** | 表间无强一致依赖，SyncWorker Task/TaskGroup 可水平扩展吞吐 |
| 增量同步 | **单链路** | 全局位点顺序语义，并行分片在 StreamRuntime 内由 LogAdapter 管理 |
...
#### 5.6.2 全量迁移（FullImport + SyncWorker）

作业配置 → JobContainer → Task 切分 → TaskGroup → Reader → Channel → Writer。编排层根据任务配置自动生成作业规格，严格 1:1 Reader-Writer 配对（借鉴 DataX 管道模型）。

#### 5.6.3 增量同步（Store + IncrSync + StreamRuntime）

源端 Binlog/Redo → Store 拉取解析 → Record 队列 → IncrSync 消费应用 → 目标库；StreamRuntime Checkpoint 持久化位点（借鉴 Flink 快照机制）。
...
编排服务 → 调度器 → 资源管理器 → Worker 节点池（SyncWorker / StreamWorker）。
...
| 层级 | 机制 | 来源 |
|------|------|------|
| Task 级 | Failover 重试（maxRetryTimes） | 借鉴 DataX |
| 作业级 | Checkpoint + Savepoint | 借鉴 Flink |
| 组件级 | Store/IncrSync HA | 借鉴 OMS |
| 数据级 | 脏数据 errorLimit + 差异订正 | 借鉴 DataX + OMS |
...
本节说明本平台与三套参考系统的**设计层关系**（借鉴其架构思想，非集成调用）：

| 设计要素 | DataX 借鉴 | Flink 借鉴 | OMS 借鉴 | 本平台实现 |
|----------|-----------|-----------|---------|-----------|
| 产品流程 | — | — | 结构→全量→增量→校验→切换 | 工作流服务 + 状态机 |
| 结构迁移 | — | — | DBCat Schema 转换 | DBCat |
| 全量管道 | Reader-Channel-Writer、Task 调度 | — | Full-Import 四阶段 | FullImport + SyncWorker |
| 增量拉取 | — | Source/Split 思想 | Store 组件 | Store + LogAdapter |
| 增量应用 | — | Checkpoint、流式运行时 | Incr-Sync、DML/DDL/HB | IncrSync + StreamRuntime |
| 数据校验 | — | — | Full-Verification | FullVerification |
| 插件体系 | Framework+Plugin SPI | Connector SPI | — | Reader/Writer + LogAdapter |
| HA/监控 | — | JobManager HA | Supervisor、组件 HA | Supervisor + HA 组件 |

| 能力 | 参考系统具备 | 本平台（自研实现） |
|------|-------------|-------------------|
| 结构迁移 | OMS | ✅ DBCat |
| 全量迁移 | DataX、OMS | ✅ FullImport（借鉴 DataX 管道） |
| 增量同步 | Flink、OMS | ✅ Store + IncrSync（借鉴 Flink 运行时） |
| 数据校验 | OMS | ✅ FullVerification |
| 端到端编排 | OMS | ✅ 控制台 + 工作流 |
| 组件 HA | OMS、Flink | ✅ Store/IncrSync HA |

详见 [OMS架构参考](OMS架构参考.md) 中的差异对照章节。

### 9.2 关键设计决策

| 决策 | 选择 | 理由 |
|------|------|------|
| 平台定位 | 自研一体化迁移平台 | 非 DataX/Flink 集成封装，形成完整产品能力 |
| 全量引擎 | 自研，借鉴 DataX 管道模型 | 吸收 Framework+Plugin、Channel 流控等成熟设计 |
| 增量引擎 | 自研，借鉴 Flink 运行时模型 | 吸收 Checkpoint、分布式调度等流式容错设计 |
| 产品骨架 | 借鉴 OMS 组件划分与流程 | 迁移阶段划分、组件命名、HA 模型经过生产验证 |
| 目标端 | 开放多种数据库 | 通用平台定位，不绑定 OceanBase |
| Record 格式 | DML/DDL/HB（借鉴 OMS） | 屏蔽源端差异，统一 IncrSync 处理 |
| 插件隔离 | ClassLoader 隔离（借鉴 DataX） | 避免插件依赖冲突 |
